<?php
    ...
    if ($result->num_rows > 0) {
        ...
        //validar rol
        $sql = "SELECT * FROM  rol WHERE rolid = '$rol'";
        $result = $conn->query($sql);//anotacion optimizar consulta
        $num_rols = $result->num_rows;
        if( $num_rols>0){
            $validar = true;
        }else{
            CloseCon($conn);
        ...
        if($validar){
            ...
            VALUES ('$usuario','$nombre','$apellidos','$telefono','$passen','$email','$fecha','$fecha_registro','$rol')";

            if ($conn->query($sql) === TRUE) {
                CloseCon($conn);
                $data['valido'] = true;
                $data['mensaje'] = 'Usuario registrado con exito';
                echo json_encode($data);
                exit;
            } else {
                CloseCon($conn);
